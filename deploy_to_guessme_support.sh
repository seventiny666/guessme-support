...
# 部署到 guessme-support 仓库

echo "=========================================="
echo "  部署到 guessme-support 仓库"
echo "=========================================="
echo ""

# 检查是否有support-page目录
if [ ! -d "support-page" ]; then
    echo "❌ 错误：找不到 support-page 目录"
    exit 1
fi

# 创建临时目录用于克隆新仓库
TEMP_DIR="/tmp/guessme-support-deploy"
...
# 复制文件
echo "📋 复制文件到仓库..."
cp support-page/index.html "$TEMP_DIR/index.html"
cp support-page/privacy.html "$TEMP_DIR/privacy.html"

# 创建terms.html
cat > "$TEMP_DIR/terms.html" << 'EOF'
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
